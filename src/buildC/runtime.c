...
// 加密文件名
static const char BINFILE[] = "oca95169.dat";

static const char TEMPDIR[] = "KA_TEMP";

// AES 参数
static const unsigned char AES_KEY[AES_KEY_LEN] = {
    // node 生成
    0x2f, 0x71, 0xfe, 0x26, 0x25, 0xcb, 0xb9, 0x74, 0x0f, 0x99, 0xa0, 0xce, 0x48, 0xe4, 0x80, 0x64, 0x28, 0xb1, 0x6a, 0x26, 0xd9, 0x6c, 0x46, 0x0a, 0xed, 0x2d, 0xec, 0x8b, 0xdb, 0x10, 0x29, 0xa7
    // node 生成
};

static const unsigned char AES_IV[AES_IV_LEN] = {
    // node 生成
    0x34, 0x90, 0x14, 0x0b, 0x4d, 0xf7, 0x0d, 0xdc, 0xb6, 0x4e, 0xe7, 0x77
    // node 生成
};

static const unsigned char AES_TAG[AES_TAG_LEN] = {
    // node 生成
    0x91, 0x10, 0x6e, 0x53, 0x56, 0xb8, 0xdd, 0x19, 0x47, 0xea, 0xf2, 0x7d, 0xe1, 0x8e, 0x47, 0xf5
    // node 生成
};
...
#ifdef _WIN32
    snprintf(path, sizeof(path), "%s\\ka_%s%08lX%04X%04X%02X%02X%02X%02X%02X%02X%02X%02X.dll",
             temp_dir, prefix,
...
#else
    // POSIX: 复用完全相同的命名格式
    snprintf(path, sizeof(path), "%s/ka_%s%08X%04X%04X%02X%02X%02X%02X%02X%02X%02X%02X.dll",
             temp_dir, prefix,
             d1, d2, d3,
             rand_bytes[8], rand_bytes[9], rand_bytes[10], rand_bytes[11],
             rand_bytes[12], rand_bytes[13], rand_bytes[14], rand_bytes[15]);
...
PHP_FUNCTION(ltdc180d)
{
    ...
    int temp_dir_len = snprintf(
        ...
        "%s/%s",
#endif,
        ext_dir, // devMode ? ext_dir : temp_root
        TEMPDIR);
...
/* {{{ proto string|false create_temp_file(string $content, string $prefix)
   ...
   注意：必须在 ltdc180d 执行成功后调用。 */
PHP_FUNCTION(fz571e31)
{
...
static const zend_function_entry runtime_functions[] =
    {
        PHP_FE(ltdc180d, NULL)
        PHP_FE(fz571e31, NULL)
        PHP_FE_END
    };
